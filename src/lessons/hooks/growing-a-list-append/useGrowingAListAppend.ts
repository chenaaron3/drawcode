...
import content from "./growing-a-list-append.md?raw";
...
export function useGrowingAListAppend(lessonId: string) {
  ...
  const task2Complete = useVariablesDefined({ orders: ["daisies", "periwinkle", "tulips"] });
  const task3Complete = useVariablesDefined({ orders: ["daisies", "periwinkle", "tulips", "roses"] });
  const task4Complete = useTerminalContainsAll(["daisies", "periwinkle", "tulips", "roses"]);
  ...
  useEffect(() => {
    ...
    startLesson(lessonId, content, [
      ...
      },
      {
        id: "append-tulips",
        title: "Append Tulips",
        description: "Jiho just received a new order for 'tulips'. Use append to add this string to orders.",
      },
      {
        id: "append-roses",
        title: "Append Roses",
        description: "Another order has come in! Use append to add 'roses' to orders.",
      },
      {
  ...
  }, [lessonId, currentTask, task1Complete, task2Complete, task3Complete, task4Complete]);
}